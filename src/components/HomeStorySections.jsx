...
import { getPortfolioImage } from "../data/media";
...
const storyBlocks = [
  {
    ...
    image: getPortfolioImage(6) || getPortfolioImage(0),
    ctaLabel: "Ver Briefs",
  ...
  },
  {
    ...
    image: getPortfolioImage(7) || getPortfolioImage(1),
  ...
  return (
    <section className="w-full border-y border-[color:var(--as-border)] bg-[var(--as-glass-soft)]">
      {storyBlocks.map((block, index) => (
        <article
          key={block.id}
          className={`group/story grid gap-6 rounded-[2rem] border border-[color:var(--as-border)] bg-[var(--as-glass-soft)] p-6 lg:grid-cols-2 lg:items-stretch ${index > 0 ? "mt-6" : ""}`}
        >
          <div className={`${block.reverse ? "order-2 lg:order-2" : "order-1 lg:order-1"}`}>
            <div className="flex h-full min-h-[18rem] items-end overflow-hidden rounded-[1.5rem]">
              {block.image ? (
                <div className="relative h-full w-full overflow-hidden">
                  <img
                    src={block.image}
                    alt={block.visualLabel}
                    className="h-full min-h-[18rem] w-full object-cover transition duration-700 ease-out group-hover/story:scale-105"
                    loading="lazy"
                  />
          ...
          >
            <div className="w-full px-2 py-4 sm:px-4 lg:px-8">
              ...
              <h2 className="mt-3 font-['Cormorant_Garamond'] text-5xl leading-[0.9] text-[var(--as-text)] sm:text-6xl">
                {block.title}
              </h2>
              ...
              <div className="mt-8">
                <Link
                  to={block.ctaTo}
                  className="inline-block rounded-full border border-[color:var(--as-border-strong)] px-6 py-3 text-xs uppercase tracking-[0.18em] text-[var(--as-text)] transition hover:bg-[var(--as-inverse-bg)] hover:text-[var(--as-inverse-text)]"
                >
                  {block.ctaLabel}
                </Link>
              </div>
